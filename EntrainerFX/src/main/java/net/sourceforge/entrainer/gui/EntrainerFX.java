...
public class EntrainerFX extends Application {
  ...
  private void setProgramItemsDisabled(boolean b) {
    boolean disable = b || soundControlPane.isPlayingEntrainerProgram()
        || soundControlPane.isRecordingEntrainerProgram();
  ...
  private void bindSocket() {
    ...
    try {
      ...
      new NotificationWindow(
          "EntrainerFX socket bound to host " + socket.getHostName() + " and port " + socket.getPortNumber());
  ...
  private void recordClicked(boolean recording) {
    ...
    fireReceiverChangeEvent(recording, MediatorConstants.RECORD_CLICKED);
  }
  ...
  private void pauseEventPerformed() {
    Thread pauseThread = new Thread() {
      public void run() {
        PauseEvent e = new PauseEvent(this,
            soundControlPane.getPause().isSelected() ? PauseEvent.PAUSE : PauseEvent.RESUME);
    ...
    fireReceiverChangeEvent(soundControlPane.getPause().isSelected(), MediatorConstants.PAUSE_CLICKED);
  }
  ...
  private void clearXmlFile() {
    fireReceiverChangeEvent(true, MediatorConstants.CLEAR_PROGRAM);
